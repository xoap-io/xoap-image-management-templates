# Windows Server 2019 — vSphere base golden image

Builds a clean **Windows Server 2019 Standard** base golden image on **vCenter / vSphere** from the
...
| 4 | *(stage + Pester)* | Uploads and runs `tests/Server2019Base.Tests.ps1` before sysprep; a failed assertion aborts the build. |
| 5 | `scripts/windows-server-Invoke_Sysprep.ps1` (VMMode) | Generalize for capture (`/generalize /oobe /mode:vm`). |

**Autounattend:** `autounattend/2019/vsphere/Autounattend-Standard.xml` (attached via floppy).

## Run in a XOAP workspace

1. Vendor the scripts: `pwsh examples/sync-demo-scripts.ps1 -Set server2019-base-vsphere`.
...
- A generalized, patched **Windows Server 2019 Standard vSphere template** (`server2019-base`) in
  the `Templates` folder, ready to clone for new VMs.
- The image passes the in-build Pester gate: correct OS build (`10.0.17763`), Sysprep present, and
