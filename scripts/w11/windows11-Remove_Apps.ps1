...
} catch { Write-Host "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] [WARN] [RemoveApps] Transcript unavailable: $($_.Exception.Message)" }
...
try {
    Write-Log "===== Remove_Apps starting ====="
    $startTime = Get-Date
    ...
    Write-Log "===== Remove_Apps complete in $([int]((Get-Date) - $startTime).TotalSeconds)s ====="
    exit 0
} catch {
