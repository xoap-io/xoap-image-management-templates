<#
.SYNOPSIS
    Configure Scheduled Tasks for Windows 10/11

.DESCRIPTION
    Creates, modifies, and manages Windows scheduled tasks for maintenance,
    backups, monitoring, and automation. Optimized for Windows 10/11
    and Packer workflows.

.NOTES
    File Name      : windows11-Configure_Scheduled_Tasks.ps1
    Prerequisite   : PowerShell 5.1 or higher, Administrator privileges
    Copyright      : XOAP.io
    
.EXAMPLE
    .\windows11-Configure_Scheduled_Tasks.ps1 -CreateMaintenanceTasks
    Creates standard maintenance tasks
    
.EXAMPLE
    .\windows11-Configure_Scheduled_Tasks.ps1 -CreateTask -TaskName "Backup" -ScriptPath "C:\Scripts\Backup.ps1" -Schedule Daily -Time "02:00"
...
} catch { Write-Host "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] [WARN] [ScheduledTasks] Transcript unavailable: $($_.Exception.Message)" }
...
function Write-LogMessage {
    ...
    $logMessage = "[$timestamp] [$levelTag] [ScheduledTasks] $Message"
...
function New-ScheduledTaskEx {
    ...
    try {
        ...
        if ($existingTask) {
            Write-LogMessage "  [WARN] Task already exists: $Name" -Level Warning
        ...
        Write-LogMessage "  [OK] Task created successfully" -Level Success
        ...
        Write-LogMessage "  [FAIL] Error creating task: $($_.Exception.Message)" -Level Error
...
function Remove-ScheduledTaskEx {
    ...
    try {
        ...
        if (-not $task) {
            Write-LogMessage "  [WARN] Task not found: $Name" -Level Warning
            return $true
        }
        
        Unregister-ScheduledTask -TaskName $Name -Confirm:$false -ErrorAction Stop
        
        Write-LogMessage "  [OK] Task removed successfully" -Level Success
        $script:TasksRemoved++
        return $true
    }
    catch {
        Write-LogMessage "  [FAIL] Error removing task: $($_.Exception.Message)" -Level Error
        $script:OperationsFailed++
        return $false
    }
}
...
function New-MaintenanceTasks {
    ...
    New-ScheduledTaskEx `
        ...
        -Execute "powershell.exe" `
        -Args "-NoProfile -ExecutionPolicy Bypass -Command `"C:\xoap-scripts\windows11-Optimize_Storage.ps1 -CleanupMode Standard`"" `
    ...
    New-ScheduledTaskEx `
        ...
        -Execute "powershell.exe" `
        -Args "-NoProfile -ExecutionPolicy Bypass -Command `"C:\xoap-scripts\windows11-Manage_Certificates.ps1 -ValidateCertificates`"" `
...
function Main {
    ...
    Write-LogMessage "Log file: $LogFile" -Level Info
    Write-LogMessage "===== Configure_Scheduled_Tasks complete in $([int]((Get-Date) - $startTime).TotalSeconds)s ====="
...
try {
    $startTime = Get-Date
    Write-LogMessage "===== Configure_Scheduled_Tasks starting ====="
    Main
}
catch {
