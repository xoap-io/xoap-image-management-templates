<#
.SYNOPSIS
    Provision Windows 10/11 Guest VM

.DESCRIPTION
    Configures Windows 10/11 VM for Packer builds including:
...
.NOTES
    File Name      : windows11-Provision_VM.ps1
    Prerequisite   : PowerShell 5.1 or higher, Administrator privileges
    Copyright      : XOAP.io
    
.EXAMPLE
    .\windows11-Provision_VM.ps1
...
} catch { Write-Host "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] [WARN] [Provision] Transcript unavailable: $($_.Exception.Message)" }
...
try {
    Write-Log "===== Provision_VM starting ====="
    $startTime = Get-Date
    ...
    Write-Log "===== Provision_VM complete in $([int]((Get-Date) - $startTime).TotalSeconds)s ====="
    exit 0
} catch {
