...
try {
    ...
    Write-Log '===== Harden_SMB_Configuration starting ====='
    ...
    try {
        ...
        Write-Log "Could not retrieve SMB configuration: $($_.Exception.Message)" -Level WARN
    }
    ...
    try {
        ...
        Write-Log "[OK] SMBv1 server protocol disabled"
        $script:ConfigurationsApplied++
        
        # Disable SMBv1 Client
        sc.exe config lanmanworkstation depend= bowser/mrxsmb20/nsi
        sc.exe config mrxsmb10 start= disabled
        Write-Log "[OK] SMBv1 client disabled"
        $script:ConfigurationsApplied++
        ...
        if ($smb1Feature -and $smb1Feature.State -eq 'Enabled') {
            ...
            Write-Log "[OK] SMBv1 Windows feature disabled"
            $script:ConfigurationsApplied++
        }
    } catch {
        Write-Log "Error disabling SMBv1: $($_.Exception.Message)" -Level WARN
    }
    ...
    try {
        ...
        Write-Log "[OK] SMBv2/SMBv3 protocol enabled"
        $script:ConfigurationsApplied++
    } catch {
        Write-Log "Error enabling SMBv2/SMBv3: $($_.Exception.Message)" -Level WARN
    }
    ...
    try {
        ...
        Write-Log "[OK] SMB encryption enabled"
        $script:ConfigurationsApplied++
        
        # Reject unencrypted access
        Set-SmbServerConfiguration -RejectUnencryptedAccess $true -Force -ErrorAction Stop
        Write-Log "[OK] Unencrypted SMB access rejected"
        $script:ConfigurationsApplied++
    } catch {
        Write-Log "Error configuring SMB encryption: $($_.Exception.Message)" -Level WARN
    }
    ...
    try {
        ...
        Write-Log "[OK] SMB signing required"
        $script:ConfigurationsApplied++
        
        Set-SmbServerConfiguration -EnableSecuritySignature $true -Force -ErrorAction Stop
        Write-Log "[OK] SMB signing enabled"
        $script:ConfigurationsApplied++
    } catch {
        Write-Log "Error configuring SMB signing: $($_.Exception.Message)" -Level WARN
    }
    ...
    try {
        ...
        if ($osVersion.Major -ge 10 -and $osVersion.Build -ge 20348) {
            ...
            Write-Log "[OK] SMB compression disabled"
            $script:ConfigurationsApplied++
        } else {
            Write-Log "SMB compression settings not available on this OS version"
        }
    } catch {
        Write-Log "Could not configure SMB compression: $($_.Exception.Message)" -Level WARN
    }
    ...
    try {
        ...
        Write-Log "[OK] Auto-disconnect timeout set to 15 minutes"
        $script:ConfigurationsApplied++
    } catch {
        Write-Log "Error configuring timeouts: $($_.Exception.Message)" -Level WARN
    }
    ...
    try {
        ...
        Write-Log "[OK] SMB1 access auditing enabled"
        $script:ConfigurationsApplied++
    } catch {
        Write-Log "Error enabling SMB1 auditing: $($_.Exception.Message)" -Level WARN
    }
    ...
    try {
        ...
        Write-Log "[OK] Registry-based SMB hardening applied"
        $script:ConfigurationsApplied++
    } catch {
        Write-Log "Error applying registry settings: $($_.Exception.Message)" -Level WARN
    }
    ...
    try {
        ...
        Write-Log "Could not display final configuration" -Level WARN
    }
    
    # Summary
    Write-Log "IMPORTANT: A system restart may be required for all changes to take effect"
    Write-Log "===== Harden_SMB_Configuration complete in $([int]((Get-Date) - $startTime).TotalSeconds)s applied=$script:ConfigurationsApplied ====="
    exit 0
