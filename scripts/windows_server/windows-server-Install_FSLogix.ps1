<#
.SYNOPSIS
    Downloads, installs and configures the Microsoft FSLogix agent for AVD / RDSH session hosts.

.DESCRIPTION
    Installs the FSLogix Apps agent (profile containers) on a Windows Server RDSH / AVD multi-session host image and applies a recommended profile-container
...
.EXAMPLE
    .\windows-server-Install_FSLogix.ps1
    Downloads and installs FSLogix, applies defaults (VHDLocations left unset with a warning).

.EXAMPLE
    .\windows-server-Install_FSLogix.ps1 -VHDLocations '\\storage\profiles'
