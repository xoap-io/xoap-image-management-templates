...
} catch { Write-Host "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] [WARN] [VMTools] Transcript unavailable: $($_.Exception.Message)" }
...
function Write-Log {
    ...
    $logMessage = "[$timestamp] [$Level] [VMTools] $Message"
...
try {
    ...
    foreach ($service in $XenServices) {
        if (Get-Service -Name $service -ErrorAction SilentlyContinue) {
            Write-Log "[OK] VMware service found: $service"
            $xenInstalled = $true
        }
    }
    ...
    if ($installerExt -eq '.msi') {
        ...
        if ($process.ExitCode -eq 0) {
            Write-Log "[OK] VMware Tools MSI installation completed successfully"
        }
        elseif ($process.ExitCode -eq 3010) {
            Write-Log "[OK] VMware Tools installation completed (reboot required)"
        }
        else {
    ...
    elseif ($installerExt -eq '.exe') {
        ...
        if ($process.ExitCode -eq 0) {
            Write-Log "[OK] VMware Tools EXE installation completed successfully"
        }
        else {
    ...
    while ($attempt -lt $MaxRetries -and -not $allServicesRunning) {
        ...
        if ($servicesFound -gt 0 -and $servicesFound -eq $servicesRunning) {
            ...
            Write-Log "[OK] All VMware services are running"
        }
        else {
    ...
    if (-not $allServicesRunning) {
        Write-Log "Warning: Not all VMware services are running after $MaxRetries attempts" -Level WARN
    ...
    if ($drivers) {
        Write-Log "[OK] Found VMware PV drivers:"
        foreach ($driver in $drivers) {
            Write-Log "  - $($driver.DeviceName) [$($driver.DriverVersion)]"
        }
    }
    else {
        Write-Log "Warning: No VMware PV drivers detected (may require reboot)" -Level WARN
    }
