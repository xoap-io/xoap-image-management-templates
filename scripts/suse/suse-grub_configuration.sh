#!/bin/bash
#===================================================================================
# Script: grub_configuration_suse.sh
# Description: Configure GRUB bootloader for SUSE/openSUSE
# Author: XOAP Infrastructure Team
# Usage: ./grub_configuration_suse.sh [--timeout SECONDS]
#===================================================================================
...
log_info "Starting GRUB configuration for SUSE..."
...
SECURITY_PARAMS="audit=1 audit_backlog_limit=8192"
...
# Set GRUB background (optional)
if [[ -f /boot/grub2/themes/openSUSE/background.png ]]; then
    if grep -q "^GRUB_BACKGROUND=" "$GRUB_DEFAULT"; then
        sed -i 's|^GRUB_BACKGROUND=.*|GRUB_BACKGROUND="/boot/grub2/themes/openSUSE/background.png"|' "$GRUB_DEFAULT"
    else
        echo 'GRUB_BACKGROUND="/boot/grub2/themes/openSUSE/background.png"' >> "$GRUB_DEFAULT"
    fi
    log_info "[OK] GRUB background configured"
fi
...
if [[ -d /sys/firmware/efi ]]; then
    log_info "Detected EFI system"
    GRUB_CFG="/boot/efi/EFI/opensuse/grub.cfg"
    
    if grub2-mkconfig -o "$GRUB_CFG" 2>&1 | tee /tmp/grub-update.log; then
        log_info "[OK] GRUB configuration updated (EFI)"
        ((CONFIGS_MODIFIED++))
    else
    ...
    if grub2-mkconfig -o "$GRUB_CFG" 2>&1 | tee /tmp/grub-update.log; then
        log_info "[OK] GRUB configuration updated (BIOS)"
        ((CONFIGS_MODIFIED++))
    else
        log_error "Failed to update GRUB configuration"
        cat /tmp/grub-update.log
