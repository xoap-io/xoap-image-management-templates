#!/bin/bash
#===================================================================================
# Script: install_docker_suse.sh
# Description: Install Docker CE for SUSE/openSUSE
# Author: XOAP Infrastructure Team
# Usage: ./install_docker_suse.sh [--version VERSION]
#===================================================================================
...
log_info "Starting Docker CE installation for SUSE..."
...
for pkg in $OLD_PACKAGES; do
    if rpm -q "$pkg" &>/dev/null; then
        ...
        zypper remove -y "$pkg" || log_warn "Failed to remove $pkg"
    fi
done
...
if zypper install -y ca-certificates curl; then
...
if [[ -f /etc/os-release ]]; then
    ...
    if [[ "$ID" == "opensuse-leap" ]]; then
        REPO_URL="https://download.docker.com/linux/opensuse/leap/$VERSION_ID"
    elif [[ "$ID" == "opensuse-tumbleweed" ]]; then
        REPO_URL="https://download.docker.com/linux/opensuse/tumbleweed"
    elif [[ "$ID" == "sles" ]]; then
        REPO_URL="https://download.docker.com/linux/sles/$VERSION_ID"
    else
        log_error "Unsupported SUSE distribution: $ID"
        exit 1
    fi
    ...
    log_error "Cannot detect SUSE version"
    exit 1
fi

# Add repository
zypper addrepo "$REPO_URL" docker-ce-stable
...
if [[ "$DOCKER_VERSION" == "latest" ]]; then
    if zypper install -y docker-ce docker-ce-cli containerd.io docker-buildx-plugin docker-compose-plugin; then
    ...
    if zypper install -y "docker-ce-${DOCKER_VERSION}" "docker-ce-cli-${DOCKER_VERSION}" \
